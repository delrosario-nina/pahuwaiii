...
document.getElementById("resetRequestForm").onsubmit = async (e) => {
  ...
  if (res.ok) {
    alert(
      "Check your email for a reset link. (In dev, check server console for the link.)"
    );
    document.getElementById("resetModal").classList.add("hidden");
...
saveEditFieldBtn.onclick = async () => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      alert(
        data.error ||
          field.charAt(0).toUpperCase() + field.slice(1) + " update failed"
      );
    }
  } catch (err) {
...
document.getElementById("changePasswordBtn").onclick = () => {
  ...
  const parent =
    editFieldInput.parentElement ||
    editFieldModal.querySelector(".p-8") ||
    editFieldModal;
